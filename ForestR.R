...
# ══════════════════════════════════════════════════════════════════
#  MÓDULO 1 — EQUAÇÕES ALOMÉTRICAS DE HIGUCHI
#  Fonte: Higuchi et al. 1997, 1998 · LMF/INPA · Manaus
# ══════════════════════════════════════════════════════════════════

#' Biomassa acima do solo — Higuchi et al. 1998
#' DAP < 20 cm: ln(AGB) = -1.754 + 2.665 * ln(DAP)
#' DAP >= 20 cm: ln(AGB) = -0.151 + 2.170 * ln(DAP)
#' @param dap DAP em cm · @return AGB em kg/árvore

biomassa_higuchi <- function(dap) {
  if (any(dap <= 0, na.rm = TRUE)) stop("DAP deve ser > 0.")
  ifelse(dap < 20,
         exp(-1.754 + 2.665 * log(dap)),
         exp(-0.151 + 2.170 * log(dap)))
}
...
resumo_parcela <- function(dados, area_parcela_ha = 1) {
  ...
  agb_ha <- (sum(biomassa_higuchi(dados$dap)) / 1000) / area_parcela_ha
  cat("\n══════════════════════════════════════════\n")
  ...
  cat("══════════════════════════════════════════\n\n")
  invisible(list(agb_ha = agb_ha, c_ha = agb_ha * 0.47))
}
  ...
  ggplot(df, aes(x = dap)) +
    ...
    annotate("label",
             ...
             label = paste0(
               ...
               "AGB ≈ ", round(sum(biomassa_higuchi(dap))/1000, 2), " Mg"
             ),
...
arvores <- data.frame(
  ...
  mutate(agb_kg = round(biomassa_higuchi(dap), 1),
         c_kg   = round(carbono_higuchi(dap),  1),
